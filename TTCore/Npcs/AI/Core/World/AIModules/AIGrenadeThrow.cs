...
public class AIGrenadeThrow : AIModuleBase
    {
        ...
        public override void Tick()
        {
            ...
            if (Parent.TryGetItem(out Throwable throwable))
            {
                if (hasLOS && !hasCollider)
                    Throw(throwable);
            }
            else
                Parent.EquipItem<Throwable>();
        }
        ...
        public void Throw(Throwable item)
        {
            ...
            if (InfiniteGrenades)
                Parent.Core.Profile.Player.AddItem(item.Type);

            item.Base.ServerProcessInitiation();
            item.Base.ServerProcessThrowConfirmation(!rClick, Parent.CameraPosition, Parent.ReferenceHub.transform.rotation, Vector3.zero);
        }
    }